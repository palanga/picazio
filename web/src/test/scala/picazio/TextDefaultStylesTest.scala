...
class TextDefaultStylesTest extends WebInterpreterSpec with Matchers {
  ...
  testRenderZIOSafe("static text with on click should have the same styles but a pointer cursor.") { (render, select) =>
    for {
      ...
      html <- select.renderedHtml
    } yield html shouldBe """<span style="padding-top: 4px; cursor: pointer;">hola</span>"""
  }
  ...
  testRenderZIOSafe("text input default styles") { (render, select) =>
    for {
      ...
      html <- select.renderedHtml
    } yield html shouldBe """<input placeholder="hola..." style="padding-bottom: 0px; padding-top: 4px;">"""
  }
  ...
  testRenderZIOSafe("button default styles") { (render, select) =>
    for {
      ...
      html <- select.renderedHtml
    } yield html shouldBe """<button style="cursor: pointer; padding-bottom: 1px; padding-top: 3px;">HOLA</button>"""
  }
  ...
  testRenderZIOSafe("text, input and button should have the same height by default") { (render, select) =>
    ...
    for {
      ...
      html <- select.renderedHtml
    } yield html shouldBe """<div style="display: flex; flex-direction: row; align-items: flex-start; justify-content: flex-start;"><span style="padding-top: 4px;">hola</span><input placeholder="hola..." style="padding-bottom: 0px; padding-top: 4px;"><button style="cursor: pointer; padding-bottom: 1px; padding-top: 3px;">HOLA</button></div>"""
  }
